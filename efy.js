export let efy_version = '23.02.01 Beta', $ = document.querySelector.bind(document), $all = document.querySelectorAll.bind(document), $create = document.createElement.bind(document), $head, $body, $root, $efy_module, efy = {}, efy_lang = {}, efy_audio = {volume: 1}, $save =()=>{},
...
/*Notify*/ $notify =(a,b,c)=>{ let d = 'alert' + Date.now();
...
/*Buttons*/ $insert($('[efy_about]'), 'afterend', $add('div', {efy_card: '', class: 'efy_about_div efy_hide_i'}, [$add('p', {efy_lang: 'sidebar_about_text'}), $add('div', {class: 'efy_flex'}, [
...
$append(b, $add('div', {id: 'efy_pickers'})); let ba = $$(b, '#efy_pickers');
    ...
    if (efy.gradient){ $root.style.setProperty(`--efy_color`, efy.gradient); $root.style.setProperty(`--efy_color_trans`, efy.gradient_trans);
        for (i=1; i < efy.color_nr + 1; i++){ let j = i, x = efy[`color${j}`], y = x.split(' '), h =  y[0], s = y[1].replace('%', ''), l = y[2].replace('%', '');
...
$$(ba, '.efy_color_add').addEventListener('click', ()=>{ let a = $$all(ba, '[efy_color]').length + 1; let c = $$(ba, '.efy_color_add'), d = $$(ba, '.efy_color_remove');
    ...
    /*Update*/ let x; x = efy[`color${a}`] = efy[`color${a-1}`] ; hsl = [`hsl(${x})`, `hsla(${x} / .3)`]; efy.color_nr++;
...
c = ['text', 'bgcol']; ['Text,0,100,50,efy_color_text', 'Background,0,100,0,efy_color_bgcol'].map((a,i)=>{
...
$insert($$(b, '[efy_color*=efy_color_text]'), 'afterend', $add('br', {}));
let cnt_clr = '#efy_sbtheme [efy_content=colors] [efy_color*=efy_color';


for (let x = ['1', '2', '_text', '_bgcol'], y = ['color1', 'color2', 'text', 'bgcol'], i=0; i < x.length; i++){
...
$$(cp, '.efy_color_picker_copy').addEventListener('click', ()=>{ let b = cs.textContent.toLowerCase(); let d = $$(a,`.efy_color_picker_${b}`).value; navigator.clipboard.writeText(d); $notify(5, 'Copied to clipboard')});
...
cs.addEventListener('click', ()=>{ if (efy_swc == 1) {$text(cs, 'RGB')} else if (efy_swc == 2) {$text(cs, 'HEX')} else if (efy_swc == 3){efy_swc = 0; $text(cs, 'HSL')} efy_swc++;
    ...
    let y = ['text', 'bgcol']; ['_text', '_bgcol'].map((x,i)=>{ if (a.getAttribute('efy_color').includes(`color${x}`)){ $root.style.setProperty(`--efy_color${x}_var`, hsl); efy[y[i]] = hsl; $save() }});

    /*Reset gradient*/ let x  = {y: '', y_trans: ''};
    /*Convert to Gradient*/ for (i=1; i < efy.color_nr + 1; i++){ x.y += `, hsl(${efy[`color${i}`]})`; x.y_trans += `, hsla(${efy[`color${i}`]} / .3)`}
...
/*Checkbox Toggles*/  'audio_status audio_click audio_hover outline text_status bgcol_status trans_filter_menu'.split(' ').forEach(x =>{ if (efy[x] == 'on'){ $(`#efy_${x}`).setAttribute('checked', '')} $(`#efy_${x}`).addEventListener('click', () =>{ if (efy[x] == 'on'){delete efy[x]} else {efy[x] = 'on'} $save() }) });
...
request.onsuccess =()=>{ let efy_count_img = 0, transaction = request.result.transaction(["images"], "readonly"), invoiceStore = transaction.objectStore("images"), getCursorRequest = invoiceStore.openCursor();
    ...
    getCursorRequest.onsuccess = e => { let cursor = e.target.result;
        if (cursor){ efy_count_img++; let req = invoiceStore.get(efy_count_img);
            ...
            /*Restore Background*/ if (efy_count_img > 0){ invoiceStore.get(bgnr).onsuccess = e => { let x = e.target.result.image, y = e.target.result.id; $text(efy_css_bgimg, `html:before, html:after {background: url(${x})!important; background-size: cover!important}`); $('.efy_img_previews [efy_bg_nr="'+y+'"]').setAttribute('efy_active','')}}
...
/*Import Settings*/ let efy_localstorage_import = $('#efy_localstorage_import'); efy_localstorage_import.addEventListener('change', ()=>{
	let file = efy_localstorage_import.files[0], read = new FileReader();
	read.onload =()=>{ Object.entries(JSON.parse(read.result)).forEach(([k,v])=>localStorage.setItem(k,v)); location.reload()}
...
/*Lang Pseudo Trigger (Temporary)*/ $append($('.efy_sidebar'), $add('i', {efy_lang: 'no_notifications', style: 'opacity: 0; pointer-events: none; position: absolute'}))
/*Prevent Default*/ $all('input[type="range"], .plus-btn, .minus-btn').forEach(a => a.addEventListener('contextmenu', ()=> event.preventDefault()));

$wait(.01, ()=>{/*Translations*/ $ready('[efy_lang]', ()=>{ $efy_lang() });
    ...
    /*No Notifications*/ $append($head, $add('style', {}, [`.efy_quick_notify_content:empty:before {content: '${efy_lang.no_notifications}'}`]));
